...
import { getPedidos, deletePedido, copiarPedido, convertirPresupuesto, getVendedores } from '@/services/fac';
...
const TIPO: Record<string, { label: string; cls: string }> = {
  ...
  C: { label: 'Contrato', cls: 'bg-purple-100 text-purple-700' },
  I: { label: 'Interno',  cls: 'bg-gray-100 text-gray-600' },
  M: { label: 'Muestra',  cls: 'bg-orange-100 text-orange-700' },
  D: { label: 'Dise\u00f1o',  cls: 'bg-teal-100 text-teal-700' },
};
...
const ESTADO: Record<string, { label: string; cls: string }> = {
  ...
  R: { label: 'Rechazado', cls: 'bg-red-100 text-red-600'     },
};
...
const COLUMNS = [
  { key: 'nro',    header: 'Nro.',     sortKey: 'nro',     headerClassName: 'w-24', cell: (p: Pedido) => p.ped_nro, cellClassName: 'font-mono text-xs text-gray-500' },
  { key: 'fecha',  header: 'Fecha',    sortKey: 'fecha',   headerClassName: 'w-28', cell: (p: Pedido) => formatDate(p.ped_fecha), cellClassName: 'text-gray-600' },
  { key: 'tipo',   header: 'Tipo',     headerClassName: 'w-24',
    cell: (p: Pedido) => {
      const t = TIPO[p.ped_tipo ?? ''] ?? { label: p.ped_tipo ?? '\u2014', cls: 'bg-gray-100 text-gray-500' };
      return <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${t.cls}`}>{t.label}</span>;
    } },
...
const PAGE_ID = 'pedidos-presupuestos';
const DEFAULTS = { fechaDesde: '', fechaHasta: '', estado: '', vendedor: '', tipo: '', search: '' };
...
export default function PedidosPage() {
  ...
  const activeFilters = [filters.fechaDesde, filters.fechaHasta, filters.estado, filters.vendedor, filters.tipo].filter(Boolean).length;
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['pedidos', queryParams],
    queryFn: () => getPedidos(queryParams),
  });
  ...
  const copiarMut = useMutation({
    mutationFn: copiarPedido,
    onSuccess: (nuevo) => {
      qc.invalidateQueries({ queryKey: ['pedidos'] });
      router.push(`/fac/pedidos/${nuevo.ped_clave}`);
    },
  });
  ...
  if (filters.tipo)       exportParams.tipo = filters.tipo;
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <h1 className="text-xl font-semibold text-gray-800">Pedidos / Presupuestos</h1>
          <p className="text-sm text-gray-500 mt-0.5">{'\u00d3'}rdenes de venta, cotizaciones, internos y muestras</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <ExportButton filename="pedidos" fetchData={() => getPedidos(exportParams)} columns={[
            { header: 'Nro.', value: (r) => r.ped_nro },
            { header: 'Fecha', value: (r) => r.ped_fecha },
            { header: 'Tipo', value: (r) => TIPO[r.ped_tipo ?? '']?.label ?? r.ped_tipo },
        ...
        <DataTable
          ...
          extraActions={(p) => (
            <>
              ...
              {p.ped_tipo === 'V' && (
                <button type="button" title="Facturar"
                  onClick={() => router.push(`/fac/facturas/nuevo?pedido=${p.ped_clave}`)}
