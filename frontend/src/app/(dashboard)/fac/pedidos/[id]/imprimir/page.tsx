...
import { getPedido } from '@/services/fac';
...
export default function ImprimirPedidoPage() {
  ...
  if (!pedido) return <div className="p-8 text-center text-gray-500">Pedido no encontrado</div>;

  const tipo = pedido.ped_tipo === 'P' ? 'PRESUPUESTO' : 'PEDIDO';
    ...
    <>
      ...
      <div className="max-w-[210mm] mx-auto p-8 print:p-0 print:max-w-none bg-white print:shadow-none">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{tipo} N.&ordm; {pedido.ped_nro}</h1>
            <p className="text-sm text-gray-500 mt-1">Fecha: {formatDate(pedido.ped_fecha)}</p>
            {pedido.ped_tipo === 'P' && pedido.ped_dias_validez && (
          ...
          <div className="text-right">
            ...
            {pedido.ped_tasa_us ? <p className="text-sm text-gray-500">Tasa U$: {pedido.ped_tasa_us}</p> : null}
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6 text-sm">
          ...
          {pedido.ped_nro_orco && (
            <div><span className="text-gray-500">Ord. Compra:</span> <span className="font-medium">{pedido.ped_nro_orco}</span></div>
          )}
          {pedido.ped_fec_entreg_req && (
            <div><span className="text-gray-500">Entrega Req.:</span> <span className="font-medium">{formatDate(pedido.ped_fec_entreg_req)}</span></div>
          )}
        ...
        <div className="flex justify-end mb-6">
          <div className="w-64 space-y-1 text-sm">
            ...
            {pedido.ped_imp_facturado ? (
              <div className="flex justify-between text-yellow-700"><span>Facturado</span><span className="tabular-nums">{fmt(Number(pedido.ped_imp_facturado))}</span></div>
            ) : null}
          </div>
        </div>
