...
// Líneas
const getLineas          = async (req, res, next) => { try { res.json(await s.getLineas({ all: req.query.all === 'true', page: parseInt(req.query.page) || 1, limit: parseInt(req.query.limit) || 20, search: req.query.search || '' })); } catch (e) { next(e); } };
...
// Marcas
const getMarcas          = async (req, res, next) => { try { res.json(await s.getMarcas({ all: req.query.all === 'true', page: parseInt(req.query.page) || 1, limit: parseInt(req.query.limit) || 20, search: req.query.search || '' })); } catch (e) { next(e); } };
...
// Rubros
const getRubros          = async (req, res, next) => { try { res.json(await s.getRubros({ all: req.query.all === 'true', page: parseInt(req.query.page) || 1, limit: parseInt(req.query.limit) || 20, search: req.query.search || '' })); } catch (e) { next(e); } };
