...
const getAll = async ({ page = 1, limit = 20, search = '', all = false } = {}) => {
  ...
  const select = `SELECT "DEP_EMPR" AS dep_empr, "DEP_SUC" AS dep_suc, "DEP_CODIGO" AS dep_codigo, "DEP_DESC" AS dep_desc FROM stk_deposito ${where} ORDER BY "DEP_EMPR", "DEP_SUC", "DEP_CODIGO"`;
  if (all) {
